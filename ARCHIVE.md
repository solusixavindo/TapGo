# TapGo Archive Index

Indeks ini mencatat lokasi dokumen penting setelah cleanup dan organisasi workspace.

## DOKU Documents

Lokasi: `docs/doku/`

- `DOKU_SETUP.md`
- `DOKU_UAT_CHECKLIST.md`
- `DOKU_PRODUCTION_WEBHOOK_UAT_CHECKLIST.md`
- `DOKU_WEBHOOK_END_TO_END_UAT.md`
- `DOKU_FINAL_PAYMENT_UAT.md`
- `DOKU_PRODUCTION_UAT_EXECUTION.md`
- `DOKU_BUSINESS_FLOW_RESPONSE.md`
- `DOKU_BUSINESS_FLOW_RESPONSE.pdf`
- `DOKU_EMAIL_RESPONSE_FINAL.md`
- `TAPGO_DOKU_MERCHANT_VERIFICATION_BOOK.pdf`
- `TAPGO_DOKU_MERCHANT_VERIFICATION_BOOK.pptx`
- `DOKU_PAYMENT_INTEGRATION_REPORT.md`

## Midtrans Documents

Lokasi: `docs/midtrans/`

- `MIDTRANS_TRANSACTION_FLOW_TAPGO_FINAL.pdf`
- `MIDTRANS_TRANSACTION_FLOW_WITH_CHANNEL_BLOCKER.pdf`
- `MIDTRANS_ONBOARDING_TRANSACTION_FLOW.pdf`
- `MIDTRANS_VERIFICATION_PACKAGE.zip`
- `MIDTRANS_VERIFICATION_PT_TAPGO_LION_INDONESIA.zip`
- `MIDTRANS_ONBOARDING_TAPGO/`
- `MIDTRANS_ONBOARDING_TAPGO_V3/`
- `MIDTRANS_ONBOARDING_TAPGO_V3_FINAL/`
- `MIDTRANS_VERIFICATION/`
- `midtrans-onboarding-flow/`
- `midtrans-transaction-flow/`

## Legal Documents

Lokasi: `docs/legal/`

- `DNB_XAVINDO_DUNS_UPLOAD/`
- `DNB_XAVINDO_DUNS_UPLOAD.zip`
- `LEGAL_COMPLIANCE_AUDIT.md`
- `SOP_MEMBERSHIP_APPROVAL.md`
- `SOP_WITHDRAWAL_PAYOUT.md`
- `SOP_REWARD_PROFIT_SHARING.md`
- `SOP_REFUND_AND_COMPLAINT.md`
- `SOP_SUPPORT_OPERATIONS.md`
- `FINAL_FINANCE_SUPPORT_SOP_REPORT.md`
- Security and role audit reports.

## Release, UAT, and Google Play Documents

Lokasi: `docs/release/`

- `RELEASE_WORKFLOW.md`
- `VERSIONING_POLICY.md`
- `AAB_V3_FINAL_RELEASE_REPORT.md`
- `AAB_V3_PRE_UPLOAD_AUDIT.md`
- `FINAL_GOOGLE_PLAY_ACCOUNT_RECOMMENDATION.md`
- `GOOGLE_PLAY_AAB_RELEASE_READINESS_AUDIT.md`
- `GOOGLE_PLAY_PRE_SUBMISSION_AUDIT.md`
- `GOOGLE_PLAY_STORE_LISTING_DRAFT.md`
- `GOOGLE_PLAY_PRODUCTION_PREP.md`
- `GOOGLE_PLAY_PRODUCTION_READINESS.md`
- `GOOGLE_PLAY_FINAL_RELEASE_CHECKLIST.md`
- `GOOGLE_PLAY_CLOSED_TESTING_PLAN.md`
- `PAYMENT_GATEWAY_STATUS.md`
- `DEPLOYMENT_GUIDE.md`
- `FINAL_PRODUCTION_DEPLOY_STEPS.md`
- `RECOVERY_GUIDE.md`
- `PRODUCTION_READINESS_CHECKLIST.md`
- `PRODUCTION_INFRASTRUCTURE_AUDIT.md`
- `INTERNAL_UAT_CHECKLIST.md`
- `CLOSED_BETA_PLAN.md`
- `FINAL_UAT_CHECKLIST.md`
- `UAT_CHECKLIST.md`
- `UAT_PHASE_A_FINAL_READINESS.md`
- `UAT_PHASE_B_FINAL_READINESS.md`
- `FOUNDER_PLATINUM_IMPLEMENTATION_REPORT.md`
- `FOUNDER_PLATINUM_ADMIN_GUIDE.md`
- User guide, FAQ, launch content, and tester guide.

## Master Blueprint Documents

Lokasi: `docs/master-blueprint/`

- `TAPGO_MASTER_BLUEPRINT_2026_2035.pdf`
- `TAPGO_MASTER_BLUEPRINT_2026_2035.pptx`
- `TAPGO_FINANCIAL_MODEL_2026_2035.xlsx`
- `TAPGO_EXECUTIVE_BOOK_2026_2035.pdf`
- `TAPGO_MASTER_BLUEPRINT_ASSUMPTIONS.md`
- `GENERATION_QA_SUMMARY.txt`

## Historical Audit Archive

Lokasi: `docs/archive/`

- Business engine audit and validation reports.
- Founder Platinum and Founder Chairman design plans.
- Anti-abuse and device fingerprint planning.
- Wallet, referral, reward, commission, and migration audits.
- Investor deck source files and final PPTX.

## Release Artifacts

Release artifacts are intentionally kept in their original artifact folders.

- `google-play-assets/releases/tapgo-user-v1.0.2-3-release.aab`
- `dist/TapGo-UAT-production-banner-premium-fixed.apk`
- `dist/TapGo-UAT-production-ui-premium-final-polish.apk`
- `dist/TapGo-UAT-production-ui-premium-2026.apk`
- `dist/TapGo-UAT-business-engine-alignment.apk`

## Important Versions

- Current product version: `v1.0.0-alpha`
- Current branch strategy: `main` + `develop`
- Google Play AAB v3: `1.0.2+3`
- DOKU integration mode: `checkout`
- DOKU production webhook: `https://api.tapgolion.id/api/v1/webhooks/doku`
- Production roadmap status: `PROJECT_STATUS.md`
- Main backend package: `apps/backend`
- Main Flutter user app: `apps/user_app`

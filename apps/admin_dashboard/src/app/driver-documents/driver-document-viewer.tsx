...
const LABELS: Record<DriverDocumentType, string> = {
  KTP: "Foto KTP",
...
/**
 * Menampilkan satu dokumen mitra driver.
 *
 ...
 * berpindah mitra.
 */
export default function DriverDocumentViewer({
  driverId,
  document
}: {
  driverId: string;
  document: DriverDocumentSummary;
}) {
  ...
  useEffect(() => {
    ...
    let created = "";
    setLoading(true);
    fetchDriverDocumentObjectUrl(driverId, document.type)
      .then((result) => {
        if (revoked) {
      ...
      .catch((caught: unknown) =>
        revoked
          ? undefined
          : setError(
              caught instanceof Error ? caught.message : "Dokumen belum dapat dibuka."
            )
      )
  ...
  }, [driverId, document.type, document.available]);
    ...
    <figure className="rounded-xl border border-slate-200 bg-white p-4 print:break-inside-avoid">
      ...
      <dl className="mt-3 space-y-1 text-xs text-slate-500">
        <div className="flex justify-between gap-4">
          ...
          <dd
            className={
              document.available ? "font-semibold text-amber-700" : "text-slate-400"
            }
          >
            {document.available
              ? remainingRetention(document.expiresAt)
              : "sudah dihapus"}
          </dd>
        </div>
        {checksum ? (
